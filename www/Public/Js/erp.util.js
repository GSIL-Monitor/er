...
//        {"id": "5", "name": "转预订单"},
	...
	formatter.hot_print_status = function(value, row, index){
    	var remark_deal_type = data["hot_print_status"];
    	for(var x in remark_deal_type){
    		if(remark_deal_type[x]["id"] == value){
    			return remark_deal_type[x]["name"];
    		}
    ...
    //平台对账对账状态
    formatter.account_check_status = function(value, row, index){
        var account_check_status = data["account_check_status"];
        for (var x in account_check_status){
            if(account_check_status[x]["id"] == value){
                return account_check_status[x]["name"];
            }
        }
    }
    ...
    formatter.get_data = function (key, type, id,isShift) {
        ...
        if (type == 'no') {
            arr[0] = {"id": "no", "name": "全部", "selected": true};
        } else if (type == 'sel') {
...
(function (){
    workTime = {};
    workTime.getWorkTimeNum = function (type) {
        var myDate = new Date();
