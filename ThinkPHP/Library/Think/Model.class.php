...
class Model {
    ...
    const PDO_ERROR             =  '未知错误,请联系管理员!'; //捕获PDO异常，返回的提示信息
    const E_ERROR               =  '捕获到Exception'; //捕获Exception，返回的提示信息
    const IMPORT_NUM            =  5000;
    const OVER_IMPORT_ERROR     =  '导入条数不能超过5000条,请分批导入!';
    const OVER_EXPORT_ERROR     =  '8:00-19:00可以导出1000条，其余时间可以导出4000条!';
    const EXPORT_CSV_ERROR      =  '导出csv文件8:00-19:00可以导出10000条，其余时间可以导出20000条!';
     ...
     */
    public function __construct($name='',$tablePrefix='',$connection='',$autoCheckFields = true) {
        // 模型初始化
		$this->autoCheckFields = $autoCheckFields;
        $this->_initialize();
        // 获取模型名称
        if(!empty($name)) {
