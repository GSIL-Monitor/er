 ...
 * @return Think\Model
 */
function D($name='',$layer='', $tablePrefix='', $connection='') {
 ...
 * @return Think\Model
 */
function M($name='', $tablePrefix='',$connection='',$autoCheckFields = true) {
    static $_model  = array();
    if(empty($connection)){
		$connection = get_db_connection();
	}
    ...
    if (!isset($_model[$guid]))
        $_model[$guid] = new $class($name,$tablePrefix,$connection,$autoCheckFields);
    return $_model[$guid];
}
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if(C('SESSION_TYPE')) { // 读取session驱动
            ...
            /*
			try{
				$interval = D('Setting/System')->getOneSysteSetting('cfg_login_interval');
			} catch (\Exception $e) {
				\Think\Log::write('get login interval error:'.$e->getMessage());
			}
			$expiration = empty($interval[0]['value']) ? 30 : $interval[0]['value'];
			$hander =   new $class($expiration);
			*/
			$hander =   new $class();
