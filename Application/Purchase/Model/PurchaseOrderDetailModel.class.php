...
{
    protected $tableName = 'purchase_order_detail';
    protected $pk        = 'rec_id';
    public function insertPurchaseOrderDetailForUpdate($data,$update = false,$options = ''){
        try {
            if(empty($data[0]))
            {
            ...
            $msg = $e->getMessage();
            \Think\Log::write($this->name."-insertPurchaseOrderDetailForUpdate-".$msg);
            E(self::PDO_ERROR);
        }
    }
    public function getPurchaseOrderDetailList($fields,$conditions=array())
    {
        try {
            ...
            $msg = $e->getMessage();
            \Think\Log::write($this->name.'-getPurchaseOrderDetailList-'.$msg);
            E(self::PDO_ERROR);
        }
        return false;
    }
    public function getPurchaseOrderDetailLogInfoLeftGoodsSpec($fields,$conditions)
    {
        try {
            ...
            $msg = $e->getMessage();
            \Think\Log::write($this->name.'-getPurchaseOrderDetailLogInfoLeftGoodsSpec-'.$msg);
            E(self::PDO_ERROR);
        }
        return false;
    }
	public function showPurchasedetail($id){
		try{
			...
			$fields =  array('pc.rec_id as id','gs.spec_no',$arrive_num,$stop_num,'pc.spec_id','gg.goods_no','gg.goods_name','gb.brand_name','gs.spec_code','gs.spec_name','gs.barcode',$num,'pc.price as cost_price','pc.discount','pc.discount AS discount_rate','pc.amount','pc.remark','cgu.name as unit_name','gs.img_url',$stock_num);
	...
	public function showPurchaselog($id){
		try{
			$field = array('he.account','pol.remark','pol.created');
			$where = array('purchase_id'=>$id);
			$data = D('Purchase/PurchaseOrderLog')->alias('pol')->field($field)->join('LEFT JOIN hr_employee he ON he.employee_id = pol.operator_id')->where($where)->order('pol.created desc')->select();
		}catch(\PDOException $e){
