...
class StockSpecLogController extends BaseController{

    public function getStockSpecLog(){
        $id_list = self::getIDList($id_list,array('tool_bar', 'tab_container', 'datagrid','more_button', 'more_content','hidden_flag', 'dialog','form'));
        $fields = get_field('Stock/StockSpecLog','stockspeclog');
        $datagrid = array(
            'id'=>$id_list['datagrid'],
            'options'=> array(
                'title' => '',
                'url'   => U("StockSpecLog/search"),
        ...
        $params=array(
            'datagrid'=>array('id'=>$id_list['datagrid'],'url' => U("SaleStockout/search")),
    ...
    public function search($page = 1, $rows = 20, $search = array(), $sort = 'created', $order = 'desc'){
        $this->ajaxReturn(D('StockSpecLog')->search($page, $rows, $search, $sort, $order));
    }
}