...
class StockoutCollectController extends BaseController
{
    public function getStockoutCollect(){
        $id_list = DatagridExtention::getIdList(array('form','tool_bar','datagrid','day_start','day_end','help_id','dailystat_start','dailystat_end'));
        $fields = D('Setting/UserData')->getDatagridField('Statistics/StockoutCollect','stockoutcollect');
        $datagrid = array(
            'id'=>$id_list['datagrid'],
            'options'=> array(
                    'title' => '',
                    'url'   => U("StockoutCollect/loadDataByCondition"),
                    'toolbar' => "#{$id_list['tool_bar']}",
                    'frozenColumns'=>D('Setting/UserData')->getDatagridField('Statistics/StockoutCollect','stockoutcollect',1),
                    'fitColumns'   => false,
                    'singleSelect'=>false,
                    'ctrlSelect'=>true
            ),
...
//        array_unshift($datagrid['fields'],$checkbox);
        ...
        $params['datagrid']['url'] = U("StockoutCollect/loadDataByCondition");
        $params['datagrid']['id'] = $id_list['datagrid'];
        $params['search']['form_id'] = 'statistics-form-stock-out';
    ...
    public function loadDataByCondition($page = 1, $rows = 20, $search = array(), $sort = 'spec_id', $order = 'desc'){
        ...
        $data = D('Statistics/StockoutCollect')->loadDataByCondition($page, $rows, $search , $sort , $order);
    ...
    public function exportToExcel(){
        ...
        try{
            ...
            D('StockoutCollect')->exportToExcel($search, $id_list);
        }
