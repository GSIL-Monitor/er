...
require_once(ROOT_DIR . '/Refund/top_tm.php');

function jstDownloadTmRefundList(&$db, $first_time, $shop, $start_time, $end_time, $save_time, &$total_refund_count, &$new_refund_count, &$chg_refund_count, &$error_msg)
{
	return jstDownloadTmRefundListImpl($db, $first_time, $shop, $start_time, $end_time, $save_time, $total_refund_count, $new_refund_count, $chg_refund_count, $error_msg);
}

function jstDownloadTmRefundListImpl(&$db, $first_time, $shop, $start_time, $end_time, $save_time, &$total_refund_count, &$new_refund_count, &$chg_refund_count, &$error_msg)
{
	$new_refund_count = 0;
	$chg_refund_count = 0;
	...
	if(!$jst_db)
	{
		...
		logx("ERROR $sid getJstDb", $sid.'/Refund' ,'error');
		return TASK_OK;
	}
	
	logx("jstRefundTianmao $shopId start_time:" . 
	...
	while($incr_end_time < $end_time)
	{
		...
		$trade_calc_sql = "select SQL_CALC_FOUND_ROWS jdp_response from jdp_tm_refund where seller_nick='" . $jst_db->escape_string($shop->account_nick);
		...
		{
			$error_msg = '数据库错误';
			logx("ERROR $sid db_fail1",$sid.'/Refund', 'error');
			return TASK_OK;
		}
		...
		if ($total_results <= $once_get)
		{
			...
			while($refund = $jst_db->fetch_array($refunds))
			{
				...
				if(!loadTmRefundImpl($db, $shop, $obj->tmall_eai_order_refund_get_response->refund_bill, $refund_list, $goods_list))
				{
					continue;
				}
			}
		...
		{
			$jst_db->free_result($refunds);
			$trade_sql = "select jdp_response from jdp_tb_trade where seller_nick='" . $jst_db->escape_string($shop->account_nick);
			...
			while ($offset >= 0)
			{
				...
				{
					$error_msg = '数据库错误';
					logx("ERROR $sid db_fail3", $sid.'/Refund','error');
					return TASK_OK;
				}
