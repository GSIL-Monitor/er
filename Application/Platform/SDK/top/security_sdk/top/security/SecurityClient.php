<?php
	...
	require_once ('SecretGetRequest.php');
	require_once ('TopSdkFeedbackUploadRequest.php');
	require_once (TOP_SDK_DIR . '/top/security_sdk/TopSdk.php');
	...
	class SecurityClient
	{
		...
		function __construct($client, $random)
		{
			...
			defined('APP_SECRET_TYPE') or define('APP_SECRET_TYPE','2');
			defined('APP_USER_SECRET_TYPE') or define('APP_USER_SECRET_TYPE','3');
		...
		*/
		...
		function toLogString($secretContext)
		{
			return $secretContext->session.','.$secretContext->encryptPhoneNum.','.$secretContext->encryptNickNum.','
			.$secretContext->encryptReceiverNameNum.','.$secretContext->encryptSimpleNum.','.$secretContext->encryptSearchNum.','
			.$secretContext->decryptPhoneNum.','.$secretContext->decryptNickNum.','.$secretContext->decryptReceiverNameNum.','
			.$secretContext->decryptSimpleNum.','.$secretContext->decryptSearchNum.','.$secretContext->searchPhoneNum.','
			.$secretContext->searchNickNum.','.$secretContext->searchReceiverNameNum.','.$secretContext->searchSimpleNum.','
			.$secretContext->searchSearchNum ;
		}
		...
		*/
		function report($secretContext)
		{
			...
			$request->setContent($this->toLogString($secretContext));
		...
		*/
		function callSecretApi($session,$secretVersion)
		{
			...
			if($response->code != 0 || !isset($response->secret) || empty($response->secret)){
