...
abstract class MarketplaceWebServiceOrders_Model
{
    ...
    private function _fromDOMElement(DOMElement $dom)
    {
        ...
        if(strstr($dom->ownerDocument->saveXML(), 'https://mws.amazonservices.com/Orders/2013-09-01'))
        {
            ...
            if (is_array($fieldType)) {
                if ($this->_isComplexType($fieldType[0])) {
                    ...
                    if ($elements->length >= 1) {
                        $arr = explode('_', $fieldType[0]);
                        if($arr[0] == 'MarketplaceWebServiceOrders' || $arr[0] == 'MarketplaceWebService')
                        	$arr[0] = TOP_SDK_DIR . '/amazon';
                        require_once (implode(DIRECTORY_SEPARATOR, $arr) . ".php");
                ...
                if ($this->_isComplexType($fieldType)) {
                    ...
                    if ($element->length == 1) {
						switch($this->_fields[$fieldName]['FieldType']) {
							case 'DateTime':
						...
						$this->_fields[$fieldName]['FieldValue'] = $data;
                    }
    ...
    {
        return preg_match('/^MarketplaceWebServiceOrders_Model_/', $fieldType) || preg_match('/^MarketplaceWebService_Model_/', $fieldType);
    }
