 ...
 */
require_once (TOP_SDK_DIR . '/amazon/Interface.php');
...
class MarketplaceWebServiceOrders_Client implements MarketplaceWebServiceOrders_Interface
{
   ...
   private function collapseWhitespace($s) {
    ...
    public function getOrder($request)
    {
        ...
        require_once (TOP_SDK_DIR . '/amazon/Model/GetOrderResponse.php');
    ...
    public function getServiceStatus($request)
    {
        if (!$request instanceof MarketplaceWebServiceOrders_Model_GetServiceStatusRequest) {
            require_once (TOP_SDK_DIR . '/amazon/Model/GetServiceStatusRequest.php');
            $request = new MarketplaceWebServiceOrders_Model_GetServiceStatusRequest($request);
        }
        require_once (TOP_SDK_DIR . '/amazon/Model/GetServiceStatusResponse.php');
    ...
    public function submitFeed($request)
	{
		...
		$httpResponse = $this->_invoke($this->_convertSubmitFeed($request), $request->getFeedContent(), $request->getContentMd5());
     ...
     */
    private function _invoke(array $parameters, $stream = NULL, $contentMD5 = null)
    {
        ...
        try {
            ...
            $parameters = $this->_addRequiredParameters($parameters, empty($stream) ? self::SERVICE_VERSION : '2009-01-01');

            $shouldRetry = true;
            $retries = 0;
            do {
                try {
                        $response = $this->_httpPost($parameters, $stream, $contentMD5);
                        if ($response['Status'] === 200) {
                            $shouldRetry = false;
                        } else {
                            if ($response['Status'] === 500 || $response['Status'] === 503) {
                            	
                            	require_once(TOP_SDK_DIR . '/amazon/Model/ErrorResponse.php');
                ...
                /* Rethrow on deserializer error */
                    ...
                    require_once (TOP_SDK_DIR . '/amazon/Exception.php');
    ...
    private function _reportAnyErrors($responseBody, $status, $responseHeaderMetadata, Exception $e =  null)
    {
        ...
        if($xml)
        {
            ...
            $errorType = $xml->Error->Type;
            if(empty($errorType)) $errorType = "Unknown";
            
            require_once (TOP_SDK_DIR . '/amazon/Exception.php');
            $ex = new MarketplaceWebServiceOrders_Exception(array ('Message' => $message, 'StatusCode' => $status, 'ErrorCode' => $code,
                'ErrorType' => $errorType, 'RequestId' => $requestId, 'XML' => $responseBody,
     ...
     *
     */
    private function _httpPost(array $parameters, $stream = NULL, $contentMD5 = NULL)
    {
		$query = $this->_getParametersAsString($parameters);
        $url = $this->_config['ServiceURL'];


        $deal_url = parse_url ($url);
        switch ($deal_url['scheme']) {
            case 'https':
                $port = isset($deal_url['port']) ? $deal_url['port'] : 443;
                break;
            default:
                $port = isset($deal_url['port']) ? $deal_url['port'] : 80;
        }
        ...
        curl_setopt($ch, CURLOPT_CAINFO, TOP_SDK_DIR . '/cacert.pem');
        ...
        if(!empty($stream) && is_resource($stream))
        {
        ...
        $resp_arr = explode("\r\n\r\n", $response, 2);
        if(!is_array($resp_arr) || count($resp_arr) != 2)
        	return array ('Status' => -1, 'ResponseBody' => $response, 'ResponseHeaderMetadata' => '');
        
        list($other, $responseBody) = $resp_arr;
        
        if($other == 'HTTP/1.1 100 Continue')
        {
        ...
        require_once(TOP_SDK_DIR . '/amazon/Model/ResponseHeaderMetadata.php');
    ...
    {   
        ...
        $parameters['Timestamp'] = $this->_getFormattedTimestamp(time());
     ...
     */
    private function _getFormattedTimestamp($stime)
    {
        return gmdate("Y-m-d\TH:i:s.\\0\\0\\0\\Z", $stime);
    }
    ...
    private function _convertListOrders($request) {
        ...
        if ($request->isSetCreatedAfter()) {
            $parameters['CreatedAfter'] =  $this->_getFormattedTimestamp($request->getCreatedAfter());
        }
        if ($request->isSetCreatedBefore()) {
            $parameters['CreatedBefore'] =  $this->_getFormattedTimestamp($request->getCreatedBefore());
        }
        if ($request->isSetLastUpdatedAfter()) {
            $parameters['LastUpdatedAfter'] =  $this->_getFormattedTimestamp($request->getLastUpdatedAfter());
        }
        if ($request->isSetLastUpdatedBefore()) {
            $parameters['LastUpdatedBefore'] =  $this->_getFormattedTimestamp($request->getLastUpdatedBefore());
        }
    ...
    private function _convertSubmitFeed($request) {
      $parameters = array();
