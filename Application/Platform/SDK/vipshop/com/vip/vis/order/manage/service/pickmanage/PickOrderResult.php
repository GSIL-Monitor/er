...
class PickOrderResult {
	...
	public function __construct($vals=null){
		...
		if (is_array($vals)){
			...
			if (isset($vals['orderSkuList'])){
				
				$this->orderSkuList = $vals['orderSkuList'];
			}
	...
	public function read($input){
		...
		while(true){
			...
			if ("orderSkuList" == $schemeField){
				
				$needSkip = false;
				
				$this->orderSkuList = array();
				$_size1 = 0;
				$input->readListBegin();
				while(true){
					...
					try{
						...
						$elem1 = new \com\vip\vis\order\manage\service\pickmanage\OrderSku();
						$elem1->read($input);
						
						$this->orderSkuList[$_size1++] = $elem1;
					}
					catch(\Exception $e){
	...
	public function write($output){
		...
		if($this->orderSkuList !== null) {
			...
			$xfer += $output->writeFieldBegin('orderSkuList');
			
			if (!is_array($this->orderSkuList)){
				
				throw new \Osp\Exception\OspException('Bad type in structure.', \Osp\Exception\OspException::INVALID_DATA);
			}
			
			$output->writeListBegin();
			foreach ($this->orderSkuList as $iter0){
