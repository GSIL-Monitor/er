...
function jstTopDownloadTradeListImpl(&$db, $first_time, $shop, $start_time, $end_time, $save_time, &$total_trade_count, &$new_trade_count, &$chg_trade_count, &$error_msg, $cb) {
    ...
    if (!$jst_db) {
        logx("getJstDb Failed", $sid . "/TradeTaobao");
        logx("ERROR $sid getJstDb", $sid . "/TradeTaobao",'error');
        return TASK_OK;
    }

    logx("jstTopDownloadShop $shopId start_time:" .
         date('Y-m-d H:i:s', $start_time) .
         " end_time:" . date('Y-m-d H:i:s', $end_time), $sid . "/TradeTaobao");

    logx('step1:'.print_r(time(),true),$sid.'/TradeTaobao');
    ...
    while ($incr_end_time < $end_time) {
        ...
        $trade_calc_sql = " from jdp_tb_trade where seller_nick='" . addslashes($shop->account_nick);

        if ($first_time)
            $trade_calc_sql .= "' and modified>='{$time_from}' and modified<='{$time_to}'";
        else
            $trade_calc_sql .= "' and jdp_modified>='{$time_from}' and jdp_modified<='{$time_to}'";
        $total_results = $jst_db->query_result_single("select count(1) $trade_calc_sql");
        if ($total_results === FALSE) //database error occur
        {
            $error_msg = '数据库错误';
            logx("ERROR $sid db_fail1", $sid . "/TradeTaobao",'error');
            return TASK_OK;
        }
        ...
        if ($total_results == 0) //无订单
        {
			...
			*/
            ...
            if ($first_time)
                $trade_sql = "select jdp_response,modified from jdp_tb_trade where seller_nick='" . addslashes($shop->account_nick) . "'";
            else
                $trade_sql = "select jdp_response,jdp_modified modified from jdp_tb_trade where seller_nick='" . addslashes($shop->account_nick) . "'";

            // get from last in case of missing trades
            while (true) {
                if ($first_time)
                    $sql = "$trade_sql and modified>='{$time_from}' and modified<='{$time_to}' order by seller_nick,modified asc limit $once_get";
                else
                    $sql = "$trade_sql and jdp_modified>='{$time_from}' and jdp_modified<='{$time_to}' order by seller_nick,jdp_modified asc limit $once_get";

                $trades = $jst_db->query($sql);
                if (!$trades) //database error occur
                {
                    $error_msg = '数据库错误';
                    logx("ERROR $sid db_fail3", $sid . "/TradeTaobao",'error');
                    return TASK_OK;
                }
				/*
                //hch日志
				...
				*/
                ...
                if ($count === FALSE) {
                    $error_msg = '数据库错误';
                    logx("ERROR $sid db_fail4", $sid . "/TradeTaobao",'error');
                    return TASK_OK;
                }
                ...
                if ($modified == $time_from) //同一时间点出现订单超过一页条?
                {
                    ...
                    {
                        logx("ERROR shop: $shopid too many trades...", $sid . "/TradeTaobao");
                        logx("ERROR $sid too many trades...", $sid . "/TradeTaobao",'error');
                        return TASK_OK;
                    } else {
                        $once_get = $once_get * 2;
                    }
        ...
        if ($save_time) {
            ...
            logx("order_last_synctime_{$shopId}".'上次抓单时间保存 jst平台 '.print_r($save_time,true),$sid. "/default");
...
function jstDownTopTradesDetail($appkey, $appsecret, $shop, &$trades, &$trade_list, &$order_list, &$discount_list, &$jst_db, &$db, &$total_trade_count, &$new_trade_count, &$chg_trade_count, &$modified, &$error_msg) {
    ...
    while ($trade = $jst_db->fetch_array($trades)) {
        ...
        if (!is_object($trade_info)) {
            logx("invalid json:" . print_r($trade, true), $sid . "/TradeTaobao",'error');
            continue;
        }
