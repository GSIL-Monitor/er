...
MAIN_LABEL:BEGIN
	...
	*/
	...
	INSERT INTO stock_spec_log(operator_id,stock_spec_id,operator_type,num,stock_num,message) 
		select @cur_uid as operator_id,ss.rec_id as stock_spec_id,2 as operator_type,sod.num,ss.stock_num-sod.num as stock_num,
		CONCAT(if(so.src_order_type = 1,'采购入库-',if(so.src_order_type = 2,'调拨入库-',if(so.src_order_type = 3,'采购退货出库-',if(so.src_order_type = 4,'盘盈入库-',if(so.src_order_type = 3,'退货入库','其他入库'))))),so.stockin_no) as message from stockin_order so left join stockin_order_detail sod on sod.stockin_id = so.stockin_id left join stock_spec ss on ss.spec_id = sod.spec_id and ss.warehouse_id = so.warehouse_id where so.stockin_id = P_StockinId;
